...
void SetVECTHisto() {
  ...
  h1_PrmNeutron   = new TH1F("h1_PrmNeutron", "h1_PrmNeutron; Parent Ipvc; Number of Events", 2490, 10, 2500);
  h1_PrmNeutron   -> SetStats(0);
  h1_ScndNeutron  = new TH1F("h1_ScndNeutron", "h1_ScndNeutron; Parent iprtscnd; Number of Events", 2490, 10, 2500);
  ...
  for (int i=0; i<3; i++) {
    h1_TrueMuMom[i] = new TH1F(TString::Format("h1_TrueMuMom_type%d", i), "True #mu Momentum; True #mu Momentum[MeV]; Number of Events", 90, 0, 900);
    h1_TruePiMom[i] = new TH1F(TString::Format("h1_TruePiMom_type%d", i), "True #pi Momentum; True #pi Momentum[MeV]; Number of Events", 90, 0, 900);
  ...
  h2_PrmVtx_XY = new TH2F("h2_PrmVtx_XY", "Truth primary vertex; x[m]; y[m]", 20, -20, 20, 20, -20, 20);
  h2_PrmVtx_XY -> SetStats(0);
  h2_PrmVtx_RZ = new TH2F("h2_PrmVtx_RZ", "Truth primary vertex; r^{2}[m]; z[m]", 20, 0, 400, 20, -20, 20);
...
void WriteVECTHisto() {
  ...
  for (int i=0; i<4; i++) {
    //h1_PrmNeutronE[i]  -> Write();
  }
  for (int i=0; i<3; i++) {
    h1_TrueMuMom[i] -> Write();
    h1_TruePiMom[i] -> Write();
    //h1_ScndNeutronE[i] -> Write();
  }

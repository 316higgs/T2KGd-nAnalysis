...
int main(int argc, char **argv) {
  ...
  for (int ientry=0; ientry<processmax; ientry++) {
    ...
    if (prmsel.Apply1RmuonSelection(evsel, numu, decayebox, eMode, eOsc, 20., 50., 400., false)) {
      ...
      bool  MuNCap = false;              //Neutron capture from mu capture?
        ...
        for (int iscnd=0; iscnd<nscndprt; iscnd++) {
              ...
              lmecscnd[ ichildidx[iscnd]-1 ]==static_cast<int>(GEANTINT::NEUTRONCAPTURE))
          {
                ...
                lmecscnd[iscnd]==static_cast<int>(GEANTINT::DECAY)) 
            {
              ...
              float d_Prm_x_MuNCap_x = PrmVtx[0] - MuNCapVtx[0];
              float d_Prm_x_MuNCap_y = PrmVtx[1] - MuNCapVtx[1];
              float d_Prm_x_MuNCap_z = PrmVtx[2] - MuNCapVtx[2];
              float d_Prm_x_MuNCap   = std::sqrt( d_Prm_x_MuNCap_x*d_Prm_x_MuNCap_x +
                                                    d_Prm_x_MuNCap_y*d_Prm_x_MuNCap_y +
                                                    d_Prm_x_MuNCap_z*d_Prm_x_MuNCap_z );
              //h1_truedistance_mu_n -> Fill(d_Prm_x_MuNCap/100.);
              h1_truedistance_mu_n -> Fill(d_Prm_x_MuNCap/100., OscProb);
            }
          }
          ...
          if (iprtscnd[iscnd]==static_cast<int>(PDGPID::GAMMA) &&
              ...
              (iprntidx[iscnd]==0 || iprntidx[iscnd]==-5) &&
          ...
          if (MuDcy) {
            NuNCap = true;
            float d_Prm_x_NuNCap_x = PrmVtx[0] - NuNCapVtx[0];
            float d_Prm_x_NuNCap_y = PrmVtx[1] - NuNCapVtx[1];
            float d_Prm_x_NuNCap_z = PrmVtx[2] - NuNCapVtx[2];
            float d_Prm_x_NuNCap   = std::sqrt( d_Prm_x_NuNCap_x*d_Prm_x_NuNCap_x +
                                                d_Prm_x_NuNCap_y*d_Prm_x_NuNCap_y +
                                                d_Prm_x_NuNCap_z*d_Prm_x_NuNCap_z );
            //h1_truedistance_nu_n -> Fill(d_Prm_x_NuNCap/100.);
            h1_truedistance_nu_n -> Fill(d_Prm_x_NuNCap/100., OscProb);
          }
      ...
      if (NuNCap) Neutrino_PrmN++;
      if (MuNCap) Neutrino_MuN++;
      if (MuNCap && NuNCap) Neutrino_Both++;
  ...
  std::cout << "Neutrino events with mu capture neutrons: " << Neutrino_MuN << std::endl;
  std::cout << "Neutrino events with primary neutrons   : " << Neutrino_PrmN << std::endl;
  std::cout << "Neutrino events with both neutrons      : " << Neutrino_Both << std::endl;

...
void NTagAnalysis::SetHistoFrame() {
  ...
  h1_TaggedN_LowPt     = new TH1F("h1_TaggedN_LowPt", "", 10, 0, 10);
  h1_TaggedN_HighPt    = new TH1F("h1_TaggedN_HighPt", "", 10, 0, 10);
  h2_allTaggedN_x_MuPt = new TH2F("h2_allTaggedN_x_MuPt", "", 80, 0, 1.5, 10, 0, 10);
...
int NTagAnalysis::GetTrueGenNBefSI(CC0PiNumu *numu, Int_t *Iorgvc) {
  int TrueNBefSI = 0;
  int mode = TMath::Abs(numu->var<int>("mode"));
    ...
    if (numu->var<int>("Ipvc", iprm)==static_cast<int>(PDGPID::NEUTRON) &&
      ...
      if (mode>=2 && mode<=10) {
        if (iprm>=3) {
          ...
          // de-excitation
          if ( iprm+1 > numu->var<int>("numnu") && Iorgvc[iprm]==2 ) {
            if (mode<31) h1_GenBefSIMom_deexc -> Fill(numu->var<float>("Abspvc", iprm), OscProb);
            else h1_GenBefSIMom_deexc -> Fill(numu->var<float>("Abspvc", iprm));
            
            if (mode<31) GenN_deex += OscProb;
            else GenN_deex++;
          }
          // nucleon FSI
          else if ( (numu->var<int>("Ipvc", Iorgvc[iprm]-1)==static_cast<int>(PDGPID::NEUTRON) ||
                     numu->var<int>("Ipvc", Iorgvc[iprm]-1)==static_cast<int>(PDGPID::PROTON)) &&
                     iprm+1 > numu->var<int>("numnu") ) {
            if (mode<31) h1_GenBefSIMom_nucFSI -> Fill(numu->var<float>("Abspvc", iprm), OscProb);
            else h1_GenBefSIMom_nucFSI -> Fill(numu->var<float>("Abspvc", iprm));
            
            if (mode<31) GenN_nucFSI += OscProb;
            else GenN_nucFSI++;
          }
          // pion FSI
          else if ( (std::fabs(numu->var<int>("Ipvc", Iorgvc[iprm]-1))==static_cast<int>(PDGPID::PIPLUS) ||
                     numu->var<int>("Ipvc", Iorgvc[iprm]-1)==static_cast<int>(PDGPID::PI0)) &&
                     iprm+1 > numu->var<int>("numnu") ) {
            if (mode<31) h1_GenBefSIMom_piFSI -> Fill(numu->var<float>("Abspvc", iprm), OscProb);
            else h1_GenBefSIMom_piFSI -> Fill(numu->var<float>("Abspvc", iprm));
            
            if (mode<31) GenN_piFSI += OscProb;
            else GenN_piFSI++;
          }
          else {
            //std::cout << "Neutrons from primary interaction?" << std::endl;
...
int NTagAnalysis::GetTrueCapNBefSI(CC0PiNumu *numu, Int_t *iprntidx, Float_t vtxprnt[][3], Float_t pprntinit[][3], Int_t *Iorgvc) {
  ...
  bool fillsrc[4];
  for (int i=0; i<4; i++) fillsrc[i] = false;
  ...
  for (int iscnd=0; iscnd<numu->var<int>("nscndprt"); iscnd++) {
        ...
        iprntidx[iscnd]<0) 
    {
      ...
      // Next gamma
        ...
        size_t indexprnt = std::distance( VtxPrntList.begin(), itrprnt );
        if (indexprnt != VtxPrntList.size()) VtxPrnt = false;  // same SI point
        else VtxPrnt = true;  // new SI point

        if (!VtxPrnt) {
          auto itrscnd = std::find(VtxScndList.begin(), VtxScndList.end(), d_vtxscnd);
          size_t indexscnd = std::distance( VtxScndList.begin(), itrscnd );
          if (indexscnd != VtxScndList.size()) VtxScnd = false;  // same neutron
          else VtxScnd = true;  // new neutron
        }
        if (VtxPrnt || (!VtxPrnt&&VtxScnd)) NewNeutron = true;
  
        // if this is gamma from new neutron, count it
        if (NewNeutron) {
          VtxScndList.push_back(d_vtxscnd);
          VtxPrntList.push_back(d_vtxprnt);
          //if (iprntidx[ iprntidx[iscnd]-1 ]==0) {
  ...
  } // truth secondary particles
  return TrueNBefSI;
}


// check which neutron corresponds to iparentNidx
void NTagAnalysis::LabelTrueCapNBefSI(CC0PiNumu *numu, Int_t *Iorgvc, int parentNidx, bool *fillsrc) {
  ...
  for (int iprm=0; iprm<numu->var<int>("Npvc"); iprm++) {
    if (numu->var<int>("Ipvc", iprm)==static_cast<int>(PDGPID::NEUTRON) &&
      ...
      if (mode>=2 && mode<=10) {
        if (iprm>=3) {
          // de-excitation
          if ( iprm+1 > numu->var<int>("numnu") && Iorgvc[iprm]==2 ) N_deex = true;
          // nucleon FSI
          else if ( (numu->var<int>("Ipvc", Iorgvc[iprm]-1)==static_cast<int>(PDGPID::NEUTRON) ||
                     numu->var<int>("Ipvc", Iorgvc[iprm]-1)==static_cast<int>(PDGPID::PROTON)) &&
                     iprm+1 > numu->var<int>("numnu") ) {
      ...
      //std::cout << "!!! parentNidx=[" << parentNidx << "] -> Primary: [" << N_prm << "] deexc: [" << N_deex << "] nucleon FSI: [" << N_nucFSI << "] pion FSI: [" << N_piFSI << "]" << std::endl; 
      // flag for fill
      if      (N_prm)    fillsrc[0] = true;
      else if (N_deex)   fillsrc[1] = true;
      else if (N_nucFSI) fillsrc[2] = true;
      else if (N_piFSI)  fillsrc[3] = true;
    }
  }
}
                                    ...
                                    std::vector<float> *DWall)  
{
  ...
  if (mode<31) AllTruthNeutrons += NTrueN * OscProb;
  else AllTruthNeutrons += NTrueN;
  ...
  for (UInt_t jentry=0; jentry<E->size(); ++jentry) {
    ...
    if (Type->at(jentry)==2) {
      ...
      if (E->at(jentry) < 6.) {
        //TruthHNeutrons++;
        if (mode<31) TruthHNeutrons += OscProb;
        else TruthHNeutrons++;
        NumTruthHNeutrons++;
      ...
      if (E->at(jentry) > 6.) {
        //TruthGdNeutrons++;
        if (mode<31) TruthGdNeutrons += OscProb;
        else TruthGdNeutrons++;
        NumTruthGdNeutrons++;
  ...
  for (int ith=0; ith<CUTSTEP; ith++) {
    ...
    for (UInt_t ican=0; ican<TagOut->size(); ++ican) {
      ...
      if (etagmode){
        ...
        //if (TagOut->at(ican)>TMVATH[ith] && etagboxin==false) NumTaggedNeutrons++;  //  <--- it can include Label=4or5 (remnant)!
        if (TagOut->at(ican)>TMVATH[ith] && etagboxin==false && (Label->at(ican)==0 || Label->at(ican)==1 || Label->at(ican)==2 || Label->at(ican)==3)) NumTaggedNeutrons++;
        ...
        if (TagOut->at(ican)>TMVATH[ith] && (Label->at(ican)==0 || Label->at(ican)==1 || Label->at(ican)==2 || Label->at(ican)==3)) NumTaggedNeutrons++;
                                      ...
                                      bool etagmode) 
{
  ...
  int NumTruthNeutronsinNlike = 0;  //Number of truth neutrons in n-like candidates of this event
  int NumTruthDecayeinNlike   = 0;  //Number of truth decay-e in n-like candidates of this event
  int NumTruthAccNoiseinNlike = 0;  //Number of truth noise in n-like candidates of this event
  ...
  for (UInt_t ican=0; ican<TagOut->size(); ++ican) {
    ...
    if (etagmode){
      ...
      if (TagOut->at(ican)>Threshold && etagboxin==false && (Label->at(ican)==0 || Label->at(ican)==1 || Label->at(ican)==2 || Label->at(ican)==3)) NumTaggedNeutrons++;
      //if (TagOut->at(ican)>Threshold && etagboxin==false && (Label->at(ican)==2 || Label->at(ican)==3)) NumTaggedNeutrons++;
      //if (TagOut->at(ican)>Threshold && etagboxin==false && (Label->at(ican)==0 || Label->at(ican)==1)) NumTaggedNeutrons++;
      //if (TagOut->at(ican)>Threshold && etagboxin==false && (Label->at(ican)!=0 && Label->at(ican)!=1 && Label->at(ican)!=2 && Label->at(ican)!=3)) NumTaggedNeutrons++;
      //if (Label->at(ican)!=0 && Label->at(ican)!=1 && Label->at(ican)!=2 && Label->at(ican)!=3) std::cout << "Label: " << Label->at(ican) << std::endl;
      ...
      if (TagOut->at(ican)>Threshold && etagboxin==false && (Label->at(ican)==0 || Label->at(ican)==1)) NumTruthAccNoiseinNlike++;
    }
    else {
      //if (TagOut->at(ican)>Threshold) NumTaggedNeutrons++;
      if (TagOut->at(ican)>Threshold && (Label->at(ican)==0 || Label->at(ican)==1 || Label->at(ican)==2 || Label->at(ican)==3)) NumTaggedNeutrons++;
  ...
  //return NumTruthNeutronsinNlike;
  //return NumTruthAccNoiseinNlike;
  //return NumTruthNeutronsinNlike+NumTruthAccNoiseinNlike;
    ...
    case 0:
      for (int ibin=0; ibin<binnumber_nu-1; ibin++) {
        if (knmtcs>=xbins[ibin] && knmtcs<xbins[ibin+1]) {
          ...
          for (int itagn=0; itagn<TaggedN-TaggedNoise; itagn++) {
            ...
            if (mode<31) TaggedN_x_knmtcs[ibin] += OscProb;
            else TaggedN_x_knmtcs[ibin]++;
      ...
      if (knmtcs>=xbins[binnumber_nu-1]) {
        for (int itagn=0; itagn<TaggedN-TaggedNoise; itagn++) {
          ...
          if (mode<31) TaggedN_x_knmtcs[binnumber_nu-1] += OscProb;
          else TaggedN_x_knmtcs[binnumber_nu-1]++;
    ...
    case 1:
      ...
      if (knmtcs>=xbins[binnumber_mu-1]) {
        for (int itagn=0; itagn<TaggedN-TaggedNoise; itagn++) {
          ...
          if (mode<31) TaggedN_x_knmtcs[binnumber_mu-1] += OscProb;
          else TaggedN_x_knmtcs[binnumber_mu-1]++;
...
void NTagAnalysis::TrueN_x_kinematics(CC0PiNumu* numu, int NTrueN, float knmtcs, double* xbins, float* TrueN_x_knmtcs, TH1F** h1, int bintype) {
  if (NTrueN==0) return;
    ...
    case 0:
      for (int ibin=0; ibin<binnumber_nu-1; ibin++) {
        if (knmtcs>=xbins[ibin] && knmtcs<xbins[ibin+1]) {
          ...
          for (int itruen=0; itruen<NTrueN; itruen++) {
            //TrueN_x_knmtcs[ibin]++;
            if (mode<31) TrueN_x_knmtcs[ibin] += OscProb;
            else TrueN_x_knmtcs[ibin]++;
      ...
      if (knmtcs>=xbins[binnumber_nu-1]) {
        ...
        for (int itruen=0; itruen<NTrueN; itruen++) {
          ...
          if (mode<31) TrueN_x_knmtcs[binnumber_nu-1] += OscProb;
          else TrueN_x_knmtcs[binnumber_nu-1]++;
    ...
    case 1:
      ...
      if (knmtcs>=xbins[binnumber_mu-1]) {
        ...
        for (int itruen=0; itruen<NTrueN; itruen++) {
          ...
          if (mode<31) TrueN_x_knmtcs[binnumber_mu-1] += OscProb;
          else TrueN_x_knmtcs[binnumber_mu-1]++;
...
void NTagAnalysis::TrueN_x_Neutronkinematics(CC0PiNumu* numu, float knmtcs, double* xbins, float* TrueN_x_knmtcs, TH1F** h1, int bintype) {
  int mode = TMath::Abs(numu->var<int>("mode"));
  float OscProb = numu->getOscWgt();
  switch (bintype) {
    case 0:
      ...
      if (knmtcs>=xbins[binnumber_n-1]) {
        ...
        if (mode<31) TrueN_x_knmtcs[binnumber_n-1] += OscProb;
        else TrueN_x_knmtcs[binnumber_n-1]++;
    ...
    case 1:
      ...
      if (knmtcs>=xbins[binnumber_n2-1]) {
        ...
        if (mode<31) TrueN_x_knmtcs[binnumber_n2-1] += OscProb;
        else TrueN_x_knmtcs[binnumber_n2-1]++;
    ...
    case 2:
      ...
      if (knmtcs>=xbins[binnumber_n3-1]) {
        ...
        if (mode<31) TrueN_x_knmtcs[binnumber_n3-1] += OscProb;
        else TrueN_x_knmtcs[binnumber_n3-1]++;
...
void NTagAnalysis::TaggedN_x_Neutronkinematics(CC0PiNumu* numu, std::vector<float> *Label, UInt_t ican, float knmtcs, double* xbins, float* TaggedN_x_knmtcs, TH1F** h1, int bintype) {
  int mode = TMath::Abs(numu->var<int>("mode"));
  float OscProb = numu->getOscWgt();
  switch (bintype) {
    case 0:
      ...
      if (knmtcs>=xbins[binnumber_n-1]) {
        ...
        if (Label->at(ican)==2 || Label->at(ican)==3) {
          ...
          if (mode<31) TaggedN_x_knmtcs[binnumber_n-1] += OscProb;
          else TaggedN_x_knmtcs[binnumber_n-1]++;
    ...
    case 1:
      ...
      if (knmtcs>=xbins[binnumber_n2-1] || knmtcs<xbins[0]) {
        ...
        if (Label->at(ican)==2 || Label->at(ican)==3) {
          ...
          if (mode<31) TaggedN_x_knmtcs[binnumber_n2-1] += OscProb;
          else TaggedN_x_knmtcs[binnumber_n2-1]++;
    ...
    case 2:
      ...
      if (knmtcs>=xbins[binnumber_n3-1]) {
        ...
        if (Label->at(ican)==2 || Label->at(ican)==3) {
          ...
          if (mode<31) TaggedN_x_knmtcs[binnumber_n3-1] += OscProb;
          else TaggedN_x_knmtcs[binnumber_n3-1]++;
